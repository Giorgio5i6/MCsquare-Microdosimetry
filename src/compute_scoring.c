...
DATA_Scoring Init_Scoring(DATA_config *config, DATA_CT *ct, int init_dose_squared){
	...
	if(config->Score_Micro == 1){
		//DEBUG
		scoring.Micro_alpha = (VAR_SCORING*)calloc(scoring.Nbr_voxels, sizeof(VAR_SCORING));
		scoring.Micro_sqrtbeta = (VAR_SCORING*)calloc(scoring.Nbr_voxels, sizeof(VAR_SCORING));
		scoring.Micro_denominator = (VAR_SCORING*)calloc(scoring.Nbr_voxels, sizeof(VAR_SCORING));
	}
	else{
		//DEBUG
		scoring.Micro_alpha = NULL;
		scoring.Micro_sqrtbeta = NULL;
		scoring.Micro_denominator = NULL;
...
void Micro_Scoring(DATA_Scoring *scoring, VAR_COMPUTE position_x, VAR_COMPUTE position_y, VAR_COMPUTE position_z, VAR_COMPUTE multiplicity, VAR_COMPUTE dE, VAR_COMPUTE Ekin, VAR_COMPUTE ion_mass, DATA_Micro data_micro, DATA_config *config){
	...
	VAR_COMPUTE Ekin_u_MeV = Ekin/UMeV/ion_mass;
	int idx = Sequential_Search(Ekin_u_MeV, data_micro.Energy_List, data_micro.Nbr_Data);
	VAR_COMPUTE alpha = Linear_Interpolation(Ekin_u_MeV, data_micro.Energy_List[idx], data_micro.Energy_List[idx+1], data_micro.Alpha[idx], data_micro.Alpha[idx+1]);
	VAR_COMPUTE sqrt_beta = Linear_Interpolation(Ekin_u_MeV, data_micro.Energy_List[idx], data_micro.Energy_List[idx+1], data_micro.Sqrt_Beta[idx], data_micro.Sqrt_Beta[idx+1]);

	scoring->Micro_alpha[index] += multiplicity * dE * alpha;
	scoring->Micro_sqrtbeta[index] += multiplicity * dE * sqrt_beta;

	//scoring->Micro_denominator[index] += multiplicity * dE;
	scoring->Micro_denominator[index] += multiplicity * dE;
...
void PostProcess_Scoring(DATA_Scoring *scoring, DATA_CT *ct, Materials *material, VAR_COMPUTE normalization, unsigned long Nbr_simulated_primaries, DATA_config *config){
	...
	if(config->Score_Micro == 1){
		for(ii=0; ii<scoring->Nbr_voxels; ii++){
			if(scoring->Micro_denominator[ii] != 0){
				scoring->Micro_alpha[ii] = scoring->Micro_alpha[ii] / scoring->Micro_denominator[ii]; //DEBUG
				scoring->Micro_sqrtbeta[ii] = scoring->Micro_sqrtbeta[ii] / scoring->Micro_denominator[ii]; //DEBUG
			}
			else {
				scoring->Micro_alpha[ii] = 0;
				scoring->Micro_sqrtbeta[ii] = 0;
			}
		}
	}
...
#pragma omp parallel for
		for(int j=0; j<Tot_scoring->Nbr_voxels; j++){
			Tot_scoring->Micro_alpha[j] += batch->Micro_alpha[j];
			Tot_scoring->Micro_sqrtbeta[j] += batch->Micro_sqrtbeta[j];
			Tot_scoring->Micro_denominator[j] += batch->Micro_denominator[j];
		}
	}
...
void Free_Scoring(DATA_Scoring *scoring){
	...
	if(scoring->Micro_alpha != NULL) free(scoring->Micro_alpha);
	if(scoring->Micro_sqrtbeta != NULL) free(scoring->Micro_sqrtbeta);
	if(scoring->Micro_denominator != NULL) free(scoring->Micro_denominator);
